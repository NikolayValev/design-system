...
- **For the free GitHub Models path** (recommended below) also add the OpenAI-compatible
  provider: `pnpm add @ai-sdk/openai`.

> **Published:** `@nikolayvalev/command-panel@1.0.0`, peer `@nikolayvalev/design-system@^2.1.0`.
...
export const dataRegistry = createDataRegistry([
  {
    ...
    description: 'Current US Treasury par-yield curve.',
    resultSchema: { type: 'array', items: { label: 'string', value: 'number' } },
    async load() {
      const res = await fetch('http://localhost:8000/api/yield-curve');
      return res.json(); // { label: string; value: number }[] — feeds a chart directly
    },
  },
  {
    id: 'bond.price',
    description: 'Clean price + yield for a bond. params: { cusip: string }.',
    resultSchema: { price: 'number', ytm: 'number' },
    async load(params) {
      const res = await fetch(`http://localhost:8000/api/price/${params?.cusip}`);
      return res.json(); // { price, ytm }
    },
  },
]);
```

> **Declare `resultSchema`.** It's shown to the model as the source's return shape so it accesses
> the data correctly — `m.data.price` for `{ price: number }`, not `m.data`. Without it the model
> guesses the shape (and may render `[object Object]`).
>
> **Charts** (`LineChart`, `BarChart`, `AreaChart`, `Donut`) expect `{ label: string; value: number }[]`.
> If a source returns that shape, the model passes it straight through — so shaping `load()` that
> way makes charts "just work."
...
// GitHub Models — free, OpenAI-compatible. Set GITHUB_MODELS_TOKEN in the env (a PAT
// with the "models" permission). See "Choosing a model" for OpenRouter / Anthropic.
...
export function POST(req: Request) {
  return createCommandPanelHandler({
    ...
    dataRegistry,
    model: github.chat(process.env.GITHUB_MODELS_MODEL ?? 'openai/gpt-4o'),
  })(req);
}
```
...
`config.model` takes any Vercel AI SDK `LanguageModel`; with none passed it falls back to
Anthropic (`claude-opus-4-8`) via `ANTHROPIC_API_KEY`. Keys always stay server-side.

**GitHub Models — free (recommended; shown in the chat route above).** Free with rate limits,
OpenAI-compatible, and `openai/gpt-4o` reliably drives the `propose_widget` tool. Create a token
with the **models** permission at <https://github.com/settings/personal-access-tokens> and set
`GITHUB_MODELS_TOKEN`. `.chat()` forces chat-completions (GitHub Models has no Responses API).

**OpenRouter / any OpenAI-compatible provider** — same shape, different base URL and key:
...
**Anthropic (zero-config default):** set `ANTHROPIC_API_KEY`, pass no `model`.
